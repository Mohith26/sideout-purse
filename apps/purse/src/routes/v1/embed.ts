...
import { okOnce } from '../../http/envelope';
import { getUser } from '../../users';
import type { V1Deps, V1Scope } from './scope';
import { param, userIdSchema } from './schemas';
import { embedTokenResource, replayedEmbedToken } from './serialize';
 ...
 * `POST /users/:id/verification`, which is the usual way to start that one. The token is
 * returned once: a replay under the same key carries `token: null` and `replayed: true`.
 */
...
export function embedRoutes(_deps: V1Deps) {
  ...
  routes.post('/tokens', async (c) => {
    ...
    const resource = embedTokenResource(issued);
    return okOnce(c, resource, replayedEmbedToken(resource), 201);
  });
