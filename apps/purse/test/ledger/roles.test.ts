...
describe('append-only enforcement at the role level', () => {
  ...
  it('the runtime role can still read and append, which is all it needs', async () => {
    ...
    expect(APPEND_ONLY_TABLES).toEqual(['journal_entries', 'journal_lines', 'audit_log', 'contest_results', 'idempotency_keys', 'eligibility_decisions', 'reconcile_runs', 'sandbox_leases']);
