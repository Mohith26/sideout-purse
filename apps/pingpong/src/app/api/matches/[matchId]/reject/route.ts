...
import { rejectResult } from '../../../../../server/matches';
...
/** Reject the reported scoreline: back to challenged, to be reported again. */
export async function POST(request: Request, context: RouteContext<{ matchId: string }>): Promise<Response> {
  return handle(request, async () => {
    ...
    const match = await rejectResult(app.db, { matchId, actor: player, now });
