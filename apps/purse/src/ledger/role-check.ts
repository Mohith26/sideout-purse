 ...
 * journal is exactly what spec 4.2.2 rule 5 forbids.
 */
export const JOURNAL_TABLES = ['journal_entries', 'journal_lines'] as const;

export type JournalTable = (typeof JOURNAL_TABLES)[number];
...
export type JournalPrivileges = {
  role: string;
  tables: Record<JournalTable, TablePrivileges>;
...
export async function runtimeRolePrivileges(sql: Sql): Promise<JournalPrivileges> {
  ...
  const rows = await sql<Array<{ table: JournalTable } & TablePrivileges>>`
    select t as "table",
    ...
    from unnest(${sql.array([...JOURNAL_TABLES])}::text[]) as t
  `;
  ...
  const tables: Record<JournalTable, TablePrivileges> = { journal_entries: absent, journal_lines: absent };
...
/** Throw unless the connected role can read and append the journal and do nothing else to it. */
