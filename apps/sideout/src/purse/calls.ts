...
/**
 * The `purse_calls` audit (spec 5.1): one row per request to Purse, written on its own
 * connection before the request leaves and completed when it returns, so a row exists
 * for a call that never came back. Always on the pool, never inside a caller's
 * transaction: a call that a rolled-back transaction caused still happened.
 */
...
export type PurseCallsPage = { calls: PurseCallView[]; nextBefore: string | null };

/** Newest first, `limit` at a time; `before` (an id) pages back, `subject` narrows to one tournament, match or user. */
export async function listPurseCalls(db: Db, options: { limit: number; before?: string | undefined; subject?: { type: string; id: string } | undefined }): Promise<PurseCallsPage> {
  const conditions = [];
