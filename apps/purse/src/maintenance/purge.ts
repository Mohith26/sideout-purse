...
import { embedTokens, IDEMPOTENCY_TTL_DAYS, idempotencyKeys } from '../db/schema';
 ...
 * documented contract. Embed tokens are five-minute artefacts and are kept a day past
 * their use or expiry for the audit trail a support question needs. Runs as the owner:
 * the runtime role cannot delete from either table.
 */
export const EMBED_TOKEN_RETENTION_MS = 24 * 3_600_000;

export type PurgeResult = { idempotencyKeys: number; embedTokens: number; before: string };
...
export async function purgeExpired(db: DbOrTx, now: Date = new Date()): Promise<PurgeResult> {
  ...
  return { idempotencyKeys: keys.length, embedTokens: tokens.length, before: keysBefore.toISOString() };
}
