...
import { failure, isApiFailure, type LadderApiError } from './errors';
...
/**
 * JSON with `bigint` rendered as a decimal string: the guard that a stray `bigint` can
 * never turn a response into a 500.
 */
...
export function fail(error: LadderApiError, status: number, init: ResponseInitLike = {}): Response {
