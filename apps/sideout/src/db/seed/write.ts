...
import { auditLog, charities, donations, matches, pools, poolTeams, sets, sponsors, teamMembers, teams, tournaments, users } from '../schema';
...
export async function writeSeed(db: Db, dataset: SeedDataset): Promise<SeedSummary> {
  await db.transaction(async (tx) => {
    ...
    await upsertAll(t, users, dataset.users);
    await upsertAll(t, tournaments, dataset.tournaments);
