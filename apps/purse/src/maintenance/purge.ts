...
import { embedSigninCodes, embedTokens, IDEMPOTENCY_TTL_DAYS, idempotencyKeys, idempotencyReservations } from '../db/schema';
 ...
 * the audit trail a support question needs; sign-in codes (ten minutes) the same. Webhook
 * deliveries and attempts are kept: they are the delivery log. Runs as the owner: the
 * runtime role cannot delete from any of these tables.
 */
export const EMBED_TOKEN_RETENTION_MS = 24 * 3_600_000;

export type PurgeResult = { idempotencyKeys: number; idempotencyReservations: number; embedTokens: number; signinCodes: number; before: string };
...
export async function purgeExpired(db: DbOrTx, now: Date = new Date()): Promise<PurgeResult> {
  ...
  return { idempotencyKeys: keys.length, idempotencyReservations: reservations.length, embedTokens: tokens.length, signinCodes: codes.length, before: keysBefore.toISOString() };
}
