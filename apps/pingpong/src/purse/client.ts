import { IDEMPOTENCY_KEY_HEADER, IDEMPOTENT_REPLAYED_HEADER, REQUEST_ID_HEADER, RETRY_AFTER_HEADER, isIdempotencyKey, type ContestKind, type EmbedFlow, type PrizeStructure } from '@purse/types';
import type { z } from 'zod';

import { PurseApiError, PurseResponseError, PurseUnreachableError } from './errors';
import { redact, redactString } from './redact';
import { contestSchema, creditSchema, embedTokenSchema, entrySchema, errorEnvelopeSchema, previewSchema, resultsSchema, scoresSchema, settlementSchema, userSchema, walletSchema } from './schemas';
 ...
 *
 * - forwards the request id (`X-Request-Id`), so a ladder request can be traced into the
 *   Purse calls it caused (spec section 10);
 * - carries an `Idempotency-Key` on every mutation (spec section 2, rule 4); the caller
 *   chooses the key, because which key a mutation reuses is the caller's rule;
 ...
 * - retries a 429 after Purse's `Retry-After`, a few times (Purse stores no 429 under an
 *   idempotency key, so the same request is safe to send again).
 *
 * This is Sideout's client with the methods the ladder does not need left out. Two
 * consumers each carrying one is what stretch item 4 exposed; the follow-up is a server
 * client in `@purse/sdk` (docs/decisions.md).
 */
export type CallSubject = { type: 'season' | 'match' | 'player'; id: string };
...
export class PurseClient {
  ...
  constructor(options: PurseClientOptions) {
    if (!SECRET_KEY_SHAPE.test(options.secretKey)) throw new RangeError('PINGPONG_PURSE_SECRET_KEY must look like sk_sandbox_... or sk_live_...');
  ...
  /** The key's environment, for a log line; never the key itself. */
  ...
  upsertUser(input: { externalId: string; displayName?: string | null }, ctx: CallContext) {
  ...
  /** What the embed's entry flow does on Purse's side, callable server to server (spec 4.7); the integration walk enters players with it. */
  enterContest(contestId: string, input: { userId: string }, ctx: CallContext) {
  ...
  // ---- The one place a request leaves the ladder for Purse ---------------------------------

  private async call<S extends z.ZodType>(method: 'GET' | 'POST', path: string, body: unknown, schema: S, ctx: CallContext): Promise<PurseResponse<z.output<S>>> {
  ...
  private async once<S extends z.ZodType>(method: 'GET' | 'POST', path: string, body: unknown, schema: S, ctx: CallContext, mutation: boolean): Promise<PurseResponse<z.output<S>>> {
